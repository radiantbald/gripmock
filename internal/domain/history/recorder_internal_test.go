...
func TestMemoryStoreDeleteRoomRemovesOnlyRoomRecords(t *testing.T) {
	...
	store.Record(history.CallRecord{Service: "svc", Method: "B", Room: "s2"})
	store.Record(history.CallRecord{Service: "svc", Method: "C", Room: ""})

	// Act
	deleted := store.DeleteRoom("s1")
