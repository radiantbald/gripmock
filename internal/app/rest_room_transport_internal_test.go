...
func TestRestAddStubRoomFromHeaderOnly(t *testing.T) {
	t.Parallel()
	...
	body := []byte(`[
		{"service":"svc.Greeter","method":"SayHello","room":"BODY","input":{"equals":{"name":"Bob"}},"output":{"data":{"message":"ok"}}}
	]`)
	...
	all := b.All()
	require.Len(t, all, 1)
	require.Equal(t, "HEADER", all[0].Room)
}
