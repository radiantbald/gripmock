...
**[完整文档](https://radiantbald.github.io/gripmock)** - 带示例的完整指南

- **Descriptor API（`/api/descriptors`）**：运行时加载编译后的 proto 描述文件（`.pb`），附带可验证的 curl 工作流：[文档](https://radiantbald.github.io/gripmock/guide/api/descriptors)
- **Upstream Modes（实验性）**：`proxy`、`replay`、`capture`，包含实用的上线迁移指导：[文档](https://radiantbald.github.io/gripmock/guide/modes)
- **Embedded SDK（实验性）**：进程内测试，支持 stubs、verification、`sdk.By(fullMethod)` 助手以及带上下文的远程校验：[文档](https://radiantbald.github.io/gripmock/guide/embedded-sdk)
- **Faker 参考**：内置 faker 的逐键说明与示例：[文档](https://radiantbald.github.io/gripmock/guide/stubs/faker)
- **OpenTelemetry + 指标**：追踪环境变量与 `/metrics` 说明：[文档](https://radiantbald.github.io/gripmock/guide/introduction/advanced-usage)
- **GitHub Actions（CI/CD）**：官方工作流 Action，可自动下载安装、启动、等待就绪并停止 GripMock：[文档](https://radiantbald.github.io/gripmock/guide/ci-cd/github-actions)
...
架构细节与基准方法见：[Performance Comparison](https://radiantbald.github.io/gripmock/guide/introduction/performance-comparison)
...
```bash
docker pull radiantbald/gripmock:v3.7.1-builder
```
...
在 CI 流水线中使用官方 Action [`radiantbald/gripmock-action`](https://github.com/radiantbald/gripmock-action) 运行 GripMock。
...
更多示例与完整 inputs/outputs 见：[GitHub Actions 指南](https://radiantbald.github.io/gripmock/guide/ci-cd/github-actions)。

## 📖 示例

在 [`examples`](https://github.com/radiantbald/gripmock/tree/master/examples) 目录查看完整示例：
...
详情见 [BSR Documentation](https://radiantbald.github.io/gripmock/guide/sources/bsr)。
...
完整指南：[gRPC Reflection Source](https://radiantbald.github.io/gripmock/guide/sources/grpc-reflection)。
...
- 📖 **[Documentation](https://radiantbald.github.io/gripmock)** - 完整指南与示例
...
- 📋 **[JSON Schema](https://radiantbald.github.io/gripmock/schema/stub.json)** - Stub 校验 schema
- 🔗 **[OpenAPI](https://radiantbald.github.io/gripmock-openapi/)** - REST API 文档
